...
export class WeaveClient extends LLMClient {
    ...
    async weaveWrapper(body: ChatCompletionCreateParamsNonStreaming) {
        let response = null;
        const extractData = weave.op(async (body: ChatCompletionCreateParamsNonStreaming) => {
            response = await this.client.chat.completions.create(body);
            return response.choices[0]!.message.content!
        });
        await extractData(body);
        return response!;
    }
